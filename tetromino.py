...
class Tetromino:
   ...
   def __init__(self, shape):
    ...
    # initialize the position of this tetromino (as the bottom left cell)
   ...
   def hard_drop(self, game_grid):
    # Taş aşağı inebildiği kadar insin
    while self.move("down", game_grid):
        continue
   def rotate(self, game_grid):
    n = len(self.tile_matrix)
    # 90 derece döndürülmüş matrix oluştur
    ...
    # Şimdi bu rotated matrix'i grid'e koyabilir miyiz kontrol et
    for row in range(n):
        for col in range(n):
            if rotated[row][col] is not None:
                pos = self.get_cell_position(row, col)
                # Pozisyon hesaplamamız doğru olmalı
                new_x = self.bottom_left_cell.x + col
                new_y = self.bottom_left_cell.y + (n - 1) - row

                # Eğer grid dışıysa veya başka taş varsa döndüremeyiz
                if not game_grid.is_inside(new_y, new_x) or game_grid.is_occupied(new_y, new_x):
                    return  # Dönüş yapma

    # Eğer buraya geldiysek dönüş geçerli, artık rotate yapabiliriz
